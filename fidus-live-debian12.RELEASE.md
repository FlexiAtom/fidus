...
`sign_release.sh` refuses a dirty or untracked source checkout and writes this binding into the manifest and provenance before signing. `verify_release_image.sh` recomputes it and fails closed before trusting the artifact. The checked-in manifest/archive are historical release artifacts; they are not claimed to bind to the current working tree, and verification must fail while this checkout is dirty. An older artifact must never be retroactively relabeled. If Docker or image reconstruction is unavailable in a future release, verification must stop at this binding/signature/metadata gate; no image identity or reproducibility claim may be inferred.

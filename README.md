...
无显示门禁应使用 vendor 与锁定配置运行完整 workspace，而不是只运行单个默认命令：
...
测试数量随 workspace 与回归用例变化，不在 README 固定一个容易过时的总数；上述门禁覆盖数学、Gate、检测器、L9/L0 端到端仿真、L1/L8/L7 估计器、协议和 Wayland 生命周期测试，其中包含 compile-fail doctest，确保坐标注入入口在类型层面不存在。

...
- **分数缩放（1.25×/1.5×/1.75×）**：校准仍成功，但残差从 0.000px 升到最高 0.308px，一致性门限余量仅剩 26%，偶发诚实失败（报错退出，不产出错误坐标）。成因是逻辑→物理的取整，[实测记录](./docs/measurements/l9-fractional-scaling.md)。**整数缩放与任意旋转均为 0.000px。**

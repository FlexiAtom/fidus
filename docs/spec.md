...
3b. **分数缩放下的稳健性** — **已测，存在真问题**：`consistency` 余量仅剩 26%，约 30 次出现 2 次 `AreaMismatch` 失败。成因与对症方向见 §4.1「已知精度边界」。**这是当前最有实测依据的改进项。**
...
7. **这个方案的各个部分，互相拆台吗？** L10 要"对数螺旋"（为消歧）又要"锁定主频"（为求解），而前者的实现方式恰恰摧毁后者。**组合方案要逐对检查相容性，不能只验证单件。**

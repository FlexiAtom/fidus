> # ⚠️ 历史存档 · 请勿据此实现
>
> **本文是 fidus 的前身草案（v0.4），已被 [`docs/spec.md`](../spec.md) 全面取代。**
> 保留它只为溯源：想知道某个设计"当初为什么这么想"时可查。
>
> **已被推翻的主要内容**（照此实现会违反当前架构铁律）：
>
> | 本文内容 | 现状 |
> |---|---|
> | L2 用户校准依赖 `globalPos()` | ❌ **违反零信任**，概率池白名单已明令禁止 |
> | L2.5 PointerProbe | ❌ 已删除（穿透与接收输入逻辑自相矛盾） |
> | L3 Beacon / L5 TUIScan | ⬜ 移出核心，归规划中的 `fidus-extras`（尚未实现） |
> | L1 Visual 与 L6 Fingerprint 分为两层 | ⬜ 已合并为单一 `L1 Fingerprint` |
> | 里程碑 P0–P7 | ⚠️ **编号与当前 P0–P3 完全不可比**（此处 P2=闭环控制、P3=C shim） |
> | `src/` 单 crate 布局、FFI(C/Python/Node) | ⬜ 已是 Rust workspace 多 crate |
> | "把校准伪装成娱乐"等桌宠 UX 哲学 | ⬜ 非定位库职责 |
>
> **已抢救进主规格的内容**（无需再从本文引用）：
> L10 原理与三大融合诡计 → `spec.md` §4.2；设计哲学 7–10 → `spec.md` 附录 B。
>
> *归档于 P2-g（2026-09）。*

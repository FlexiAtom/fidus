...
next_step: 合成图片 fixture 已接入无显示门禁；源码绑定工具和隔离输出重建脚本已实现；Docker 构建因 sandbox bridge/网络权限失败，未生成当前 revision 的正式归档，旧 manifest 不得改写为当前 HEAD；保持两项挂起，不执行真实 compositor 异常实验或 push
```

...
当前可在无显示环境完成的 P5 代码、协议、fake adapter、fixture、锁模型和 supervisor 模型门槛已完成；`fidus-test` 与 runner 回归必须持续保持全绿。剩余 `real-pending` 项均属于真实 compositor/宿主异常路径，不能通过 fake 证据替代，且真实桌面错误处理单点验证按项目进程附件保持挂起。

跨机器发布验证是独立的项目级挂起事项，不属于本表的 P5 本机实现证据。

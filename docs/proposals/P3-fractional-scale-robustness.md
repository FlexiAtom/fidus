...
> **状态：提案（待人工审阅）** — 按 AGENTS §9/§10，提案先给人审，不直接实现。
> 日期：2026-09 · 里程碑：P3
...
- [ ] 真实桌面（有窗口活动）下 scale=1 与 1.25 各 15 次，**失败 0 次**
- [ ] 仿真回归测试在改动前**确实失败**（变异验证）
- [ ] scale=1 残差仍为 **0.000px**，现有 98 测试全绿
- [ ] `cargo clippy` / `cargo doc` 零警告
- [ ] 每个新增判据都有 AGENTS §1 的三段式失效模式注释
- [ ] 实机验证**按 §4.7 协议**，日志留档

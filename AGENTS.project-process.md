...
3. **执行 P5 后全量审查**：本轮可提交的实现范围已收口；提交后对整个项目执行审查，范围按本文件第 3 节执行。
4. **后续可选：抽离正式 Niri parser adapter**：只有需要统一 Rust/shell 解析时再提出独立变更，不把 parser 与 compositor 控制耦合。
5. **处理挂起事项**：只有获得适当环境和明确授权后，分别恢复跨机器发布验证与真实桌面错误处理单点验证。
6. **审查通过后再决定发布**：push 或发布仍需单独确认，不因全量审查通过而自动执行。
